...
async def exam(
    ...
    pages = [
        {
            "name": "Daily % Change",
            "desc": """In this task, review the list of stocks provided in the table and determine the daily percentage change between the <b>Open</b> and <b>Close</b> prices for each entry. Input your calculated value into the corresponding cell under the <b>Daily % Change</b> column, which is clearly highlighted for your convenience.<br>
            <span style="color:#555;font-weight:500;">After entering your answer for the first row, you can efficiently apply the same calculation to all remaining rows by dragging the fill handle down (or by copying the input downward), ensuring consistency and saving time—just as you would in a typical Excel workflow.</span><br>
            <span style="color:#b28c0b;font-weight:600;">For added convenience, you may double-click the fill handle to automatically populate the column for all relevant rows.</span>""",
